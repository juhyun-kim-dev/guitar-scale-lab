...
> 個人開発 / Next.js・React・TypeScript / 外部API・DB・音源ファイル不使用
...
外部APIやデータベースを使わず、理論計算から音声合成までをブラウザ内で完結させています。
...
sequenceDiagram
    ...
    participant Theory as music-theory.ts
    participant Audio as audio.ts

    User->>UI: キー / スケールを選択
    UI->>Theory: ルート音とスケールタイプを渡す
    Theory-->>UI: 構成音・コード・指板上の位置を返す
    UI-->>User: 一覧・コード・指板を同時に更新
    User->>UI: 音・コード・Strumをクリック
    UI->>Audio: 弦とフレット位置・演奏設定を渡す
    Audio-->>User: 周波数を計算し、合成した音を再生
```
